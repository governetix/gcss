...
class VisualDropdown extends Component
{
    public $triggerText;
    public $triggerIcon;
    public $triggerType; // button o link
    public $position; // bottom-left, bottom-right, top-left, top-right
    public $offset; // margen entre el trigger y el menú
    ...
    public function __construct(
        ...
        // Propiedades de los ítems del menú
        $this->itemText = $itemText ?? config('gcss.dropdowns.item_defaults.text', 'text-gray-700');
    ...
    {
        // Clases para el contenedor principal del dropdown
        $containerClasses = 'relative inline-block text-left';

        // Clases para el menú desplegable
        $menuClasses = implode(' ', [
            'absolute z-50', // z-50 para asegurar que esté por encima de otros elementos
            $this->menuBg,
            $this->menuShadow,
            $this->menuRounded,
            $this->menuBorder,
            $this->menuPadding,
            $this->offset, // Margen entre el trigger y el menú
        ]);
        ...
        // Clases para los ítems del menú (se usarán en la vista Blade para cada <a> o <button>)
        $itemClasses = implode(' ', [
            'block w-full text-left', // Asegura que los ítems ocupen todo el ancho y el texto esté a la izquierda
            $this->itemText,
            $this->itemHoverBg,
            $this->itemHoverText,
            $this->itemPadding,
            'whitespace-nowrap', // Evita que el texto del ítem se rompa
        ]);
